...
if docker ps -a --format "table {{.Names}}" | grep -q "es8113\|es-nl2dsl"; then
    echo "📦 Found existing containers, starting them..."
    docker-compose start
else
    echo "🆕 No existing containers found, creating new ones..."
    docker-compose up -d
fi
...
echo "$CELERY_PID" > "$PROJECT_ROOT/backend/.celery.pid"
echo "$DJANGO_PID" > "$PROJECT_ROOT/backend/.django.pid"  
echo "$REACT_PID" > "$PROJECT_ROOT/frontend/.react.pid"
